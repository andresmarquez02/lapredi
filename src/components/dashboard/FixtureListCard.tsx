...
export function FixtureListCard({
  ...
  return (
    <div className="flex flex-1 flex-col rounded-3xl border border-border-subtle bg-surface p-6">
      ...
      {fixtures.length === 0 ? (
        <p className="flex flex-1 items-center justify-center text-sm text-text-tertiary">{t("noFixturesMatch")}</p>
      ) : (
        <ul className="flex max-h-180 flex-1 flex-col gap-3 overflow-y-auto pr-1">
              ...
              <li key={fixture.id}>
                <div
                  ...
                  className={`w-full cursor-pointer rounded-2xl border p-4 text-left transition-colors ${
                    isSelected ? "border-accent-blue/40 bg-surface-raised" : "border-transparent hover:bg-surface-raised"
                  }`}
                >
